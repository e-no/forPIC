...
#define IR_SENSOR 	RA5		/* IR PORT defination */
...
void interrupt t0intr(void) {
    ...
    } else if (TMR0IF) { /* check the interrupt on change flag */
        interruptOnChangeIsr(); /* interrupt on change has been detected call the isr */
        TMR0IF = 0; /* clear the interrupt on chage flag */
    }
}
...
void main() {
    ...
    ANSELbits.ANS = 0; /* all pin are Digital */
